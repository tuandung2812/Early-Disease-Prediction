...
def generate_code_and_target_prior(target_prefix,code_map, code_adj):
    print('generating code-target prior matrix ...')
    hf_list = np.array([cid for code, cid in code_map.items() if code.startswith(target_prefix)])
    print(hf_list)
    prior_rows = code_adj[hf_list]
    print(prior_rows.shape)

    # prior_rows = np.mean(prior_rows,axis=0)
    prior_rows = prior_rows.transpose()
    # for i in hf_list:
    #     prior_rows[i] = 0
    print(prior_rows.shape)
    # print(pri)
