...
class ModelEmbeddings(nn.Module):
    def __init__(self, embed_size, vocab_size):
        ...
        self.source = nn.Embedding(
            vocab_size, self.embed_size, padding_idx=src_pad_token_idx)
...
class NMT_tran(nn.Module):
    def __init__(self, hidden_size, head_num, embedding_size,vocab_size, encoder_layers, dropout_rate=0.2):
        ...
        c = copy.deepcopy
        self.model_embeddings = ModelEmbeddings(self.hidden_size, vocab_size)
    ...
    def encode(self, notes_padded, src_mask):
        X = self.model_embeddings(notes_padded)
        X = self.position(X)
